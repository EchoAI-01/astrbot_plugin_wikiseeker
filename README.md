...
| 配置项 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `wiki_sites` | string(JSON) | 维基百科(中/英)+Minecraft+Fandom/灰机示例 | 站点列表，JSON 数组，详见下文 |
...
| `proxy` | string | `""` | 全局网络代理（如 `http://127.0.0.1:7890`），仅对 `use_proxy:true` 的站点生效 |
...
| `use_proxy` | | 该站点是否走全局 `proxy`（维基百科/Fandom 等被墙站设 `true`，灰机等国内站不填） |

> **默认已内置 5 个站点**：维基百科中文/英文、Minecraft Wiki，以及 Terraria Fandom、饥荒灰机两个**示例站**（标注「示例」，可改子域接入任意 Fandom `xxx.fandom.com/api.php` 或灰机 `xxx.huijiwiki.com/api.php`）。维基百科与 Fandom 默认 `use_proxy:true`。所有站点都能配多个，按需增删。
>
> **巴哈姆特**（`gamer.com.tw`）无公开搜索 API、非 MediaWiki，未内置；如需接入可尝试 `docsite`（若该板块提供 `sitemap.xml`）或 `http_api`（若有 JSON 搜索接口）。
...
- **检索失败的两种处理**：
  - **技术性错误**（网络超时、HTTP 4xx/5xx、解析失败、字段映射配错等）：**只记日志**，不把错误细节回给 LLM——该站点静默跳过，不影响回复。
  - **真正未命中**（接口正常但无匹配条目）：返回「未检索到相关内容」的占位提示给 LLM，由 LLM 自行决定如何向用户表达（避免凭记忆编造）。
- **网络代理**：维基百科、Fandom 等站点在部分网络环境需代理访问。配置全局 `proxy` 后，给对应站点设 `use_proxy:true` 即走代理；灰机等国内站保持不填则直连，互不影响。注意部分站点（如灰机 Wiki）对爬虫 UA 有反爬限制，可能返回 403——此时按上一条只记日志、不阻塞回复。
- **关键词预搜索有整体超时**：`on_llm_request` 钩子在主流程中被同步等待（框架无超时），故插件自带 `keyword_trigger_timeout`（默认 8s）整体预算并对命中的多个站点**并发**检索；超时则用已就绪的部分结果、跳过未完成的站点，**优先保证不阻塞回复**。注入内容按 `max_inject_chars`（默认 4000）截断。
- **检索结果注入方式**：优先注入到用户消息尾部并标记为临时内容（不写入会话历史、稳住服务端自动前缀缓存）；旧版本 AstrBot 缺少该能力时自动退回追加 `system_prompt`。
- **Skill 经验闭环**：`update_wiki_skill` 写入的经验会在后续检索时**自动回注**到上下文，纯聊天场景下也生效（不依赖 Computer Use shell）。经验按站点 + general 过滤，回注条数与长度有上限，写入做了去重、上限与原子落盘。
...
- 本插件由 **AI 编写**：在与人类的结对开发会话中，由 Anthropic Claude（Opus 系列，模型标识 `claude-opus-4-8`）完成需求梳理、源码调研、编码实现、测试与文档撰写。
- 开发日期：**2026-06-18**。
- 大致耗时：**约 2–3 小时**（含 AstrBot 源码调研、v1 MediaWiki 实现、v2 多后端扩展、联调测试与文档）。该时长以开发当日文件时间戳为参考，仅为估算。
- 人类负责需求定义、关键设计决策的拍板与验收；AI 负责具体实现。

## 免责声明

- 本插件按「**现状**」（AS IS）提供，**不附带任何明示或暗示的担保**，包括但不限于适销性、特定用途适用性与稳定性。
- **使用本插件所产生的一切风险与后果，由使用者自行承担。** 作者与编写本插件的 AI 不对因使用、无法使用或误用本插件而导致的任何直接或间接损失负责。
- 本插件会按你的配置访问第三方服务（调用各类 Wiki / 笔记 / 文档站的 API、抓取网页、向知识库写入数据等）。请你自行：
  - 遵守各知识源、网站及 API 提供方的服务条款、使用政策、`robots.txt` 与速率限制；
  - 妥善保管在 `headers` 等配置中填写的 **API 密钥 / Token**，避免在公开场合（含本仓库配置、日志、截图）泄露；
  - 对检索结果的准确性自行判断，AI 生成的回答仍可能出错。
- 因使用本插件引发的任何合规、版权、隐私或安全问题，均与作者及编写 AI 无关。
